...
- **Member Removal Cascade**: When a member is removed via `removeMember()`, they must be stripped from all expense participant lists, any direct payments involving that member (as sender or receiver) must be removed, and if the deleted member was the payer (`from`), the payer must be reassigned to a valid remaining member.
